...
function woocommerce_pay_init()
{
    ...
    class WC_Gateway_Pay extends WC_Payment_Gateway
    {
        ...
        public function process_payment($order_id)
        {
            ...
            $transaction_request->setOrderId('Pay10_OID_'.date('dmyHis').rand(1000,9999)."_".$order->id);
        ...
        public function check_pay_response()
        {
            ...
            if (isset($response['RESPONSE_CODE'])) {
                if ($response['RESPONSE_CODE'] == '000' && $response['STATUS'] == 'Captured') {
                    $order->add_order_note('Order ID: ' . $order_id . '. ' . $response['RESPONSE_MESSAGE'] . '. Transaction Reference: ' . $response['TXN_ID']." ".'PAY10 Order Id :'.$response['ORDER_ID']);
                    ...
                    $order->add_order_note('Order ID: ' . $order_id . '. ' . $response['RESPONSE_MESSAGE'] . '. error:' . $response['RESPONSE_CODE']." ".'PAY10 Order Id :'.$response['ORDER_ID']);
    ...
    add_filter('woocommerce_payment_gateways', 'add_pay_gateway');
     ...
     //custom function for validating cashfree
   ...
   function my_plugin_action() {
    ...
    $data = new WC_Gateway_Pay();
    $pay_id=$mode=$data->pay_id;
    $mode=$data->mode;
    $sandbox = $mode;
    $payId = $pay_id;
    $pgRefNo = $_GET['order_id'];
    $orderToken =$_GET['order_token'];
    //use this function to get status enquiry
    $status = paytenCheckoutStatusEnquiry($sandbox,$payId,$pgRefNo,$orderToken);
    $string = explode('_', $status['orderId']);
    $platform_order_id = array_pop($string);
    $return_url = trailingslashit(home_url()).'?wc-api=WC_Gateway_Pay&order_id='.$platform_order_id;
   ...
   function paytenCheckoutStatusEnquiry($sandbox,$payId,$pgRefNo,$orderToken){

        $data= array("payId" => $payId, 'pgRefNo' => $pgRefNo, 'orderToken' => $orderToken); echo "<br>";

        if ($sandbox == 'yes') {
                        $url = "https://uat.pay10.com/pgws/api/v1/transaction/getStatus";
                    } else {
                        $url = "https://secure.pay10.com/pgws/api/v1/transaction/getStatus";
                    }
